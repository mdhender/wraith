...
func main() {
	...
	fmt.Printf("info: completed in %+v\n", time.Now().Sub(st))
}
...
func run() error {
	...
	fp, err := os.OpenFile("playtestSetup.txt", os.O_CREATE|os.O_TRUNC|os.O_WRONLY, 0600)
	if err != nil {
		return err
	}
	return s.Report(fp, 18)
}

func (s *Store) Report(w io.Writer, spNo int) error {
...
type Skills struct {
	...
	Gravitics     int `json:"gravitic,omitemptys"`
